...
class yobit(object):
    ...
    @staticmethod
    @backoff.on_exception(backoff.expo, (requests.exceptions.Timeout, requests.exceptions.ConnectionError), max_tries=5)
    def __api_query_public(method, pair=None, options=None):
        ...
        return requests.get(request_url, timeout=1).json()
    ...
    def __api_query_trade(self, method, options=None):
        ...
        with open(nonce_file, 'r+') as inp:
            options['nonce'] = int(inp.read())
            inp.seek(0)
            inp.write(str(options['nonce'] + 1))
            inp.truncate()
        ...
        return requests.post(request_url, data=options, headers=headers, timeout=1).json()
    ...
    def trade_history(self, pair, from_start=0, count=1000, from_id=0, end_id=0,
